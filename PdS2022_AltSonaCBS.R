##############################################################################/
##############################################################################/
#Analysis of Alternaria solani CBS
##############################################################################/
##############################################################################/
...
AltSolCBS<-read.table("data/2022_alternaria_solani_CBS.txt",
                      header=TRUE,stringsAsFactors=TRUE,sep=";")
AltSolCBS$pest_sa_id<-paste(AltSolCBS$pest_sa_id,AltSolCBS$synerg_id,sep="-")
AltSolCBS$pest_sa_id<-as.factor(AltSolCBS$pest_sa_id)
...
datamyc<-AltSolCBS[AltSolCBS$lect_echec!=1,]
...
pdf(file="output/plot_AltSolaCBS.pdf",width=7)
  ...
  if(dim(data_subSA[!(data_subSA$ech_id %in% SA_rez),])[1]!=0) {
    ...
    for (i in 1:dim(table(SA.dat$ech_id))[1]) {
      ...
      temp.m1<-drm(rslt_03~dose,
                   data=tempdat,
                   fct=LL.4())
      plot(temp.m1,ylim=c(0,110),xlim=c(0,50),pch=19,
...
write.table(CompRez, file="output/results_AltSolaCBS.txt",
